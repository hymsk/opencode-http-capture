...
- npm 分发 ESM JavaScript，包根入口只导出插件函数，不导出内部辅助函数；`prepack` 重新构建，发布文件采用白名单，不包含本地安装脚本、测试或真实捕获数据。
- npm 安装通过 OpenCode 配置的 `plugin` 数组声明包名和版本，宿主自动安装；卸载移除对应配置项并重启。消费者无需编译或执行安装脚本。
- 本地符号链接安装继续支持。迁移到 npm 前先卸载旧符号链接，不同时启用两种方式；安装方式不改变默认关闭及显式启用的安全边界。
...
构建和测试要求 Node.js 24+。`npm test` 先构建 JavaScript 并对产物测试，再检查真实 npm tarball 的文件白名单，在隔离临时目录离线安装并按包名导入，覆盖关闭、block、record、all、非法值和安装去重。模拟测试不替代实际 OpenCode/Bun 宿主验收。构建使用内置类型擦除 API，不承诺静态类型检查。
